...
def plot_city_pollutants(df):
    df_filtered = df[["city", "pollutant_id", "pollutant_avg"]]
    plt.figure(figsize=(15, 8))
    sns.set_style("whitegrid")
    sns.scatterplot(data=df_filtered, x="city", y="pollutant_avg", hue="pollutant_id", alpha=0.7, s=100)
...
if __name__ == "__main__":
    filepath = 'Pollutant_Radar.csv'
    df, label_encoders = load_and_clean_data(filepath)
    train_models(df, target_column='pollutant_id')
    plot_city_pollutants(df)
    plot_aqi(pd.read_csv(filepath))  # Use original data with strings like 'PM10'
